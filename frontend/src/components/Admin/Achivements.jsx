...
import {
  addAchivements, deleteAchivement, getUser
...
const Achivements = () => {
  ...
  const submitHandler = async(e) => {
    e.preventDefault();
    await dispatch(addAchivements(title, url, image));
    dispatch(getUser());
  };
  ...
  return (
      ...
      <div className="w-[50%] max-800px:w-[80%] rounded-md bg-[#ffffffec] shadow-2xl my-2">
        ...
        >
          <div className=" flex flex-col w-[60%] max-1000px:w-full m-auto gap-6 ">
            <p className=" text-[20px]">ACHIVEMENTS</p>
            <div className=" bg-pink-300 rounded-lg flex flex-col p-4 justify-center  items-center gap-8">
              <motion.input
                ...
                className=" outline-none h-[52px] w-full bg-[#f3ecf2f0]  rounded-lg placeholder:text-[#2a2a2a] px-4 text-[20px]"
              />
              <motion.input
                initial={{ opacity: 0, x: -400 }}
                animate={{ opacity: 1, x: 0 }}
                ...
                className=" outline-none h-[52px] w-full bg-[#ebe5eaf0]  rounded-lg placeholder:text-[#313131] px-4 text-[20px]"
              />
              <motion.input
                initial={{ opacity: 0, x: -400 }}
                animate={{ opacity: 1, x: 0 }}
                ...
                className=" bg-[#e11192] text-white rounded-[30px] cursor-pointer p-[1vmax] active:bg-[#880446] border-none"
              />
            </div>
    ...
    {user &&
      user.achivements &&
      user.achivements.map((i) => (

        <div key={i._id} className=' bg-white m-2 text-center rounded w-[300px] min-h-[150px] h-auto'>
         
          
          <Typography variant="body1" style={{fontWeight:600, fontSize:"25px",marginBottom:"20px"}}>
            {i.title}
          </Typography>
          <Button style={{
            margin:"auto",
